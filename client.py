...
class Client:
    ...
    def run_console(self):
        while True:
            print(
                f"You can enter two types of requests:\n1. Get road points by road id, type: 1 <road_id>\n2. Get all road ids, type: 2\n3. Exit program: q"
            )
            request_text: list[str] = input().split()
            try:
                if request_text[0] == "1":
                    ...
                    self.mq_manager.send_message(
                        "manager", {"command": COMMAND_QUIT, "data": None}
                    )
                    return
                else:
    ...
    def get_road_by_id(self, road_id: int):
        self.mq_manager.send_message(
            "manager", {"command": COMMAND_ROAD_BY_ID, "data": road_id}
        )
    ...
    def get_all_roads(self):
        self.mq_manager.send_message(
            "manager", {"command": COMMAND_ALL_ROADS, "data": None}
        )
        data = self.mq_manager.get_message("client")
